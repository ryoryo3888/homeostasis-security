...
def make_manifest(seed_label, seed_value, condition, pair_index, source_world_hash, prereg_hash=None):
    ...
    manifest = {
        ...
        'turns': {'start': 1, 'end': 29, 'turn_count': 29, 'turn_length_days': 30},
        ...
        'public_world_bulletin_schedule': [public_world_bulletin(turn) for turn in range(1, 30)],
...
def main():
    ...
    prereg = {
        ...
        'hypotheses': {
            ...
            'null': '外交通信の有無による主要アウトカム差は観測されない、またはTURN29まで両条件とも物理支援が成立しない可能性がある。'
        },
        ...
        'turns_fixed': {'start': 1, 'end': 29, 'turn_count': 29},
        'conditions': {
        ...
        'event_schedule': EVENT_SCHEDULE,
        'public_world_bulletin': {'definition': 'Identical public crisis/event bulletin visible in both conditions. Does not create a diplomatic channel or assign responsibility.', 'schedule': [public_world_bulletin(turn) for turn in range(1, 30)]},
        'primary_outcomes': [
            'TURN from crisis onset to first physical support dispatch; save as not established if absent by TURN29',
            'TURN from crisis onset to first physical support arrival; save as not established if absent by TURN29',
            'TURN from crisis onset to homeostasis recovery condition; save as TURN29まで未回復 if absent',
            'unmet need at TURN29',
        ...
        'success_failure_definitions': {
            'successful_run': 'All planned 29 TURN x 12 leader decisions complete, world-law settlement runs, final manifest written.',
            ...
            'substantive_non_establishment': 'No dispatch/arrival/recovery by TURN29. This is a valid result, not technical failure.',
        },
        'stop_conditions': ['Do not run extra turns beyond TURN29','Stop a run on schema/runtime/provider failure and preserve evidence','Stop whole batch if same technical failure repeats in 3 consecutive runs before diagnosis','Do not alter conditions after observing results','Do not count proposal or speech as physical support'],
        'model': MODEL,
        'generation_settings': GENERATION_SETTINGS,
