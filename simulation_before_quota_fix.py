...
def evaluate_metrics(
    ...
    prompt = f"""
あなたは国家間相互作用シミュレーションの観測評価器です。
A国・B国の意思決定には介入せず、すでに起きた相互作用だけを評価してください。

以下の情報を総合し、現在のシステム状態を0〜100の整数で評価してください。

評価項目:
- homeostasis: 外乱があっても安定を維持・回復できる程度
- tension: 対立・警戒・エスカレーション圧力の強さ
- misperception_risk: 相互の認識不足・誤解が衝突につながる危険
- trust: 相手の行動を予測し、協調可能だとみなせる程度
- resilience: 新たな問題が起きても関係を修復・維持できる程度
...
{{
  "homeostasis": 0,
  "tension": 0,
  "misperception_risk": 0,
  "trust": 0,
  "resilience": 0,
  "overall_impact": "このTURNの相互作用によってシステム全体に生じた影響を、観測可能な事実に基づいて1〜2文で記述"
}}

0は最低、100は最高です。
上の0は出力形式の例であり、実際の評価値は今回の相互作用から判断してください。
""".strip()
    ...
    while True:
        try:
            ...
            required = numeric_required | {"overall_impact"}

            if set(metrics.keys()) != required:
                raise ValueError(f"評価キーが不正です: {metrics.keys()}")
            ...
            overall_impact = str(metrics["overall_impact"]).strip()
            if not overall_impact:
                raise ValueError("overall_impact が空です")
            metrics["overall_impact"] = overall_impact
