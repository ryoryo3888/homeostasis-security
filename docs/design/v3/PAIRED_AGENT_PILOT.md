...
One paired run, two TURNs per arm, eight countries. Seed 17 is a model
sampling parameter, not a claim of exact provider reproducibility. All pairs
use the same synthetic initial world and network, frozen observation, transfer
law, solver, available opportunities and decision limits. The Gemini prompt is fixed within its arm. Each pair has
...
Maximum 32 generation attempts and 32 countTokens calls; one count then one
...
resume. The fixed .artifacts/v3-paid-pilot-20260919-phase-contract directory and durable SQLite
...
32 × ((37000+2048)×0.30 + 1536×2.50)/1e6 = **$0.4977408**.
Reserve another $0.14 for the nine prior generation attempts: combined conservative estimate **$0.6377408**.
...
unchanged. One short pair cannot support significance or broad causal claims.
...
## Reviewed consent-contract correction

The first current-model run received eight initiatives and one consent response.
The latter repeated an initiative in `decisions`, violating the core rule that
consent cannot introduce late decisions. Offline exact-response replay reproduced
`LATE_DECISION_FORBIDDEN`; no TURN was adopted. All nine original responses remain
unaltered under `.artifacts/v3-paid-pilot-20260919-current`.

The reviewed new protocol constrains consent `decisions` with `maxItems: 0` and
explicitly explains outgoing transfer direction (several original public reasons
mistook an outgoing offer for acquisition). It starts a separate pair from genesis;
it does not reuse or edit failed answers. Scope is reduced to one seed / two TURNs
per arm to keep cumulative estimated spending within $1. This is an exploratory
corrected interface pilot, not a replication under the old prompt.
